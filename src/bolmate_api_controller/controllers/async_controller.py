...
async def get_api_key_async(*, api_key_id: ID) -> Result:
    async with _semaphore, async_bolmate_connection(isolate=True, auto_commit=False) as database:
        ...
        if api_key.use_oauth:
            result = await _request_oauth_api_key(api_key=api_key)
        else:
            result = await _request_legacy_api_key(api_key=api_key)
        if result.success and result.api_key:
            ...
            await database.execute(DEACTIVATE_QUERY, {'api_key_id': api_key.api_key_id})
        return result


async def _request_oauth_api_key(*, api_key: APIKey) -> Result:
    now = PyDateTime()
    ...
    for _ in range(MAX_REQUEST_ATTEMPTS):
        try:
            ...
            result = await _process_auth_response(api_key=api_key, aio_response=aio_response, token_type='OAuth')
            last_result = result
...
async def _request_legacy_api_key(*, api_key: APIKey) -> Result:
        ...
        try:
            ...
            async with (bearer_limiter.ratelimit('legacy_bearer', delay=True),
                        aiohttp.request(method='POST', url=request_data.url, headers=request_data.headers,
                                        timeout=ClientTimeout(total=request_data.timeout)) as response):
                aio_response = await AsyncResponse.build(response=response)
            result = await _process_auth_response(api_key=api_key, aio_response=aio_response, token_type='legacy')
            last_result = result
            if result.success or not result.retryable:
                return result
            last_exception = None
...
async def _process_auth_response(*, api_key: APIKey, aio_response: AsyncResponse,
    ...
    response_text = aio_response.text()
    if status == 200:
